...
describe('Log Controller', function() {
    ...
    it('Log up with an existing mail account', async () => {
        const user = {
            name: uuid4(),
            lastName: uuid4(),
            mail: uuid4(),
        ...
        const resultGet = await users.findOne({name: user.name})
        expect(resultGet.mail).toEqual(user.mail);
        const resultError = await common.getErrorAsyncRequest(axios.post("http://localhost:" + settings.port + "/logup", user));
        expect(resultError.e).toEqual("Mail already in use.")
    ...
    it('Log in with an invalid password', async () => {
        let user = {
            name: uuid4(),
            lastName: uuid4(),
            mail: uuid4(),
